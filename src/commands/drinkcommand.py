...
class DrinkCommand(Command):
    ...
    def __volts(self, s):
        # if ( "," in s or "." in s):
        t = s.replace('\u0025', '').replace('\u002E', '').replace('\u002C', '')  # s.translate(None, '.,%')
        if (t.isdigit()):
            return float(s.replace('\u0025', '').replace('\u002C', '.'))  # s.translate(None, '%'))
        return False
    ...
    def handle(self, message):
        ...
        if (ipt == 0):  # if just !juo
            ...
            if (self.__volts(words[0]) and words[1].isdigit()):
                ...
                values['name'] = name + " " + \
                    words[0].replace('\u002C', '.').replace('\u0025', '') + "% " + str(words[1]) + "ml"
            else:
                # Syötä !juo [vahvuus(%)] [tilavuus(ml)] [nimi], esim. !juo 4.8 330 olut"
                msg = "Virheelliset parametrit!"
        else:
            msg = "Virhe jossain :("
        ...
        if (msg == ''):
            ...
            try:
                ...
                logging.error(f"DrinkCommand: {error}")
